          ...
          <Space direction='vertical'>
            <Space>
              <PopupConfirm
                ...
                selector={<Button disabled={select?.length === 0 || typeof (select) === 'undefined'} onClick={() => btnReject.current?.open()}>배치 취소 선택</Button>}
              />
              <PopupConfirm
                ref={btnAcept}
                onSubmit={() => handleButton('Approved', btnAcept.current?.close()!)}
                title='승인'
                content='탐색하고 싶습니까 ?'
                selector={<Button disabled={select?.length === 0 || typeof (select) === 'undefined'} onClick={() => btnAcept.current?.open()}>선택의 대량 승인</Button>}
              />
            </Space>
            <Space>
              ...
              <ButtonAction status={status} setStatus={setStatus} title='Approved' label='대기' />
            </Space>
          </Space>
        </Space>}
